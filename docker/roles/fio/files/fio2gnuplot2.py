...
def find_file(path, pattern):
	...
	for file in os.listdir(path):
		...
		# Let's consider this file
                ...
                p = re.compile('(.*)_bw\.\d+\.log')
                print file
                task = p.match(file).group(1)

                if not task in fio_data_file:
                    fio_data_file[task] = []
                    tasks.append(task)

		fio_data_file[task].append(os.path.join(path, file))

	return fio_data_file, tasks
...
'''% (title,mode))
		...
		global_disk_perf = sum(disk_perf.values(), [])
		...
		if len(fio_data_file) > 1:
			...
			compare_raw.write(",\\\n'%s' using 2:3 with linespoints title '%s'" % (tmp_filename,file))
			compare_smooth.write(",\\\n'%s' using 2:3 smooth csplines title '%s'" % (tmp_filename,file))
			compare_trend.write(",\\\n'%s' using 2:3 smooth bezier title '%s'" % (tmp_filename,file))

		png_file=file
                raw_filename = "%s-2Draw" % (png_file)
                smooth_filename = "%s-2Dsmooth" % (png_file)
                trend_filename = "%s-2Dtrend" % (png_file)
                avg  = average(disk_perf[file])
                f.write("call \'%s/graph2D.gpm\' \'%s' \'%s\' \'%s\' \'%s\' \'%s\' \'%s\' \'%s\' \'%f\'\n" % (gpm_dir,title,tmp_filename,file,raw_filename,mode,smooth_filename,trend_filename,avg))
                pos = pos +1
...
def compute_temp_file(fio_data_file,disk_perf,gnuplot_output_dir, min_time, max_time):
	...
	blk_size=0
        nb_files = 0
	for task, task_files in fio_data_file.iteritems():
                files[task] = []
                for file in task_files:
                    files[task].append(open(file))
                    nb_files+=1
		pos = len(files) - 1
		tmp_filename = "%sgnuplot_temp_file.%d" % (gnuplot_output_dir,pos)
		temporary_files.append(tmp_filename)
		gnuplot_file=open(tmp_filename,'w')
		temp_outfile[task] = gnuplot_file
	...
	while True:
		current_line=[]
		nb_empty_files=0
		for mytask, myfiles in files.iteritems():
                        s = [0,0,0,0]
                        elements = 0
                        for f in myfiles:
                            tmp=f.readline().replace(',',' ').split()

                            if not tmp:
                                nb_empty_files+=1
                                tmp="-1, 0, 0, 0".replace(',',' ').split()
		...
		for line in enumerate(current_line):
			...
			time, perf, x, block_size, task = line[1]
			if (blk_size == 0):
				try:
			...
			if ((float(time)>(float(min_time)*1000)) and ((int(time) < (int(max_time)*1000)) or max_time==-1)):
					disk_perf[task].append(int(perf))
					perfs.append("%d %s %s %s"% (index, time, perf, task))

		# If we reach this point, it means that all the traces are coherent
		for p in enumerate(perfs):
			index, perf_time,perf, task = p[1].split()
			temp_outfile[task].write("%s %.2f %s\n" % (index, float(float(perf_time)/1000), perf))


	for file in files.values():
          for f in file:
		f.close()
	for file in temp_outfile.values():
                file.close()
	return blk_size
...
def compute_math(fio_data_file, title,gnuplot_output_filename,gnuplot_output_dir,mode,disk_perf,gpm_dir):
	...
	for task in fio_data_file:
#		print disk_perf[disk]
	    	min_file.write("# Task %s was coming from %s\n" % (task,fio_data_file[task]))
	    	max_file.write("# Task %s was coming from %s\n" % (task,fio_data_file[task]))
	    	average_file.write("# Task %s was coming from %s\n" % (task,fio_data_file[task]))
	    	stddev_file.write("# Task %s was coming from %s\n" % (task,fio_data_file[task]))
		avg  = average(disk_perf[task])
		variance = map(lambda x: (x - avg)**2, disk_perf[task])
		standard_deviation = math.sqrt(average(variance))
#		print "Disk%d [ min=%.2f max=%.2f avg=%.2f stddev=%.2f \n" % (task,min(disk_perf[task]),max(disk_perf[task]),avg, standard_deviation)
		average_file.write('%s %d\n' % (task, avg))
		stddev_file.write('%s %d\n' % (task, standard_deviation))
		local_min=min(disk_perf[task])
		local_max=max(disk_perf[task])
		min_file.write('%s %d\n' % (task, local_min))
		max_file.write('%s %d\n' % (task, local_max))
...
def print_help():
    ...
    print '-I           or --inputdir <dir>    : The directory where log files are' 
...
def main(argv):
    ...
    try:
	    opts, args = getopt.getopt(argv[1:],"ghkbivo:d:t:p:G:m:M:I:",['bandwidth', 'iops', 'pattern', 'outputfile', 'outputdir', 'title', 'min_time', 'max_time', 'gnuplot', 'Global', 'help', 'verbose','keep', 'inputdir'])
    ...
    fio_data_file, tasks=find_file(input_dir, pattern)
    ...
    for task, file in fio_data_file.iteritems():
	print ' |-> %s' % task 
	if "_bw" in file[0] :
