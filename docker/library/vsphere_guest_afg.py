...
def add_cdrom(module, s, config_target, config, devices, default_devs, type="client", vm_cd_iso_path=None, disk_number=0, key=0):
    # Add a cd-rom
    ...
    if ide_ctlr:
        ...
        if type == "iso":
            ...
            cd_ctrl.set_element_key(key)
            cd_ctrl.set_element_controllerKey(ide_ctlr.get_element_key())
            cd_ctrl.set_element_unitNumber(int(disk_number))
...
def create_vm(vsphere_client, module, esxi, resource_pool, cluster_name, guest, vm_extra_config, vm_hardware, vm_disk, vm_nic, vm_hw_version, state):
    ...
    if 'vm_cdrom' in vm_hardware:
        ...
        cdrom_num = 0
        cdrom_key = 0
        for cdrom in sorted(vm_hardware['vm_cdrom'].iterkeys()):
            try:
                cdrom_type = vm_hardware['vm_cdrom'][cdrom]['type']
            except KeyError:
                vsphere_client.disconnect()
                module.fail_json(
                msg="Error on %s definition. cdrom type needs to be aloo afg"
                " specified." % cdrom)
            if cdrom_type == 'iso':
               try:
                   cdrom_iso_path = vm_hardware['vm_cdrom'][cdrom]['iso_path']
               except KeyError:
                   vsphere_client.disconnect()
                   module.fail_json(
                   msg="Error on %s definition. cdrom iso_path needs"
                   " to be specified." % cdrom)
        # Add a CD-ROM device to the VM.
            print " configured option cdrom %s , type %s, iso_path %s." % (cdrom, cdrom_type, cdrom_iso_path)
            add_cdrom(module, vsphere_client, config_target, config, devices,
            default_devs, cdrom_type, cdrom_iso_path, cdrom_num, cdrom_key)
	    cdrom_num = cdrom_num + 1
            cdrom_key = cdrom_key + 1
    if vm_nic:
