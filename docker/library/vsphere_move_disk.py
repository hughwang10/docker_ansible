...
def main():
    ...
    try:
        ...
        vm = search.FindByInventoryPath( vsphere_datacenter + '/vm/' + folder_new + '/' + vm_new )
        if vm == None:
          vm = search.FindByDatastorePath(dc, '[' + vsphere_datastore + '] ' + vm_new + '/' + vm_new + '.vmx')

        vmold = search.FindByInventoryPath( vsphere_datacenter + '/vm/' + folder_old + '/' + vm_old )
        if vmvmold == None:
          vmold = search.FindByDatastorePath(dc, '[' + vsphere_datastore + '] ' + vm_old + '/' + vm_old + '.vmx')
...
# vim.vm.device.VirtualDeviceSpec.FileOperation.destroy )
        ...
        for disk_number1 in disk_ids, disk_path in disk_files:
        # the details we will need to connect the disk:
          disk_fullpath = '[' + vsphere_datastore + '] ' + disk_path
